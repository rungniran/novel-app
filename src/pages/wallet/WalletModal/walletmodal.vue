...
<!-- :style="`height: ${Height}px`" -->
        ...
        <div class="walllet-conthai" >
          ...
          <div class="walllet-page" id="page2">
            <div style="display: flex; align-items: center; justify-content: flex-start; grid-gap: 20px;" ><i @click="leftPage(1)" class="fas fa-arrow-left"></i> {{titlePage2}}</div>
             <div v-if="dataObj.typeHowto === 'true_wallet'"> 
              <input class="phone" placeholder="เบอร์โทรศัพท์" type="number" v-model="phoneNumber"/>
              <div v-for="item, index in page2" :key="index"  @click="omise(item); dataObj.setCoin = item.wallet" class="walllet-list">
            ...
            <div v-else>
              <div v-for="item, index in page2" :key="index" @click="omise(item); dataObj.setCoin = item.price_amount" class="walllet-list">
                 <div  class="walllet-con-coin">
                    <img :src="$path.image('coin-gold.png')" width="20px" /> 
                    {{item.coin_amount}}
                  </div> 
          ...
          <div class="walllet-page" id="page3">
            <div  style="display: flex; align-items: center; justify-content: flex-start; grid-gap: 20px;" ><i @click="leftPage(2)" class="fas fa-arrow-left"></i> {{titlePage2}}</div>
            <div v-if="dataObj.typeHowto === 'Bank'">
              ...
              <div class="walllet-con-bank-accout">
                  ...
                  <img src="https://www.kasikornbank.com/SiteCollectionDocuments/about/img/logo/logo.png" width="20%" style="margin: 20px 0px">
...
export default Vue.extend({
  ...
  methods:{
    async leftPage(page, item){
      let w = -360
      let wallletConthai = document.getElementsByClassName("walllet-conthai")[0] 
       if(page === 1){
        ...
        onCreateTokenSuccess: (nonce) => {
          console.log(nonce);
           if (nonce.startsWith("tokn_")) {
            ...
            (res) => {
              alert(`เติมเหรียญสำเร็จ +${item.coin_amount} เหรียญ`,'success')
    ...
    cleckpp(res){
      ...
      if(this.$route.name === 'Wallet'){
        setTimeout(() => {
          Gatway.postService('/customers/payment-gateway/prompt-pay/check', {id:res.data.data.id}).then(
            (rescheck) => {
