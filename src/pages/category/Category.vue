    ...
    <div class="nv-box-white nv-mt-40 con-Category">
      <div class="filter">
        <div class="con-grop">
         ...
         <div class="grop">
              <input type="checkbox" :checked="translated" @change="TypeNovel(dataTypeNovel.translated)"> 
              <span>นิยายแปล</span>
            </div>
            <div class="grop">
              <input type="checkbox" :checked="mysafe" @change="TypeNovel(dataTypeNovel.mysafe)"> 
      ...
      <div class="novel-category">
        <router-link v-for="(item, index) in pageOfItems" :key="index" :to="'/novel/'+ item.id" class="box-crad">
          <div class="con-detail">
            <div>
              <div class="title line-1">{{item.title}} </div>
              <!-- <div class="title-cate">แอคชั่นไซไฟ</div> -->
              <span style="display: flex; gap: 10px; align-items: center;"> <NovelStar :rating="Math.round (item.avg_star)"/></span>
      ...
      <div class="card-footer">
        <jw-pagination :items="exampleItems" @changePage="onChangePage"  :pageSize="8" :maxPages="5"></jw-pagination>
      </div>
    </div>
...
export default Vue.extend({
  ...
  methods:{
    async getNovelType(){
     ...
     filterCategory(){
       this.getRecommend()
      var searchResult = this.category.filter(word => word['id'].indexOf(this.textCategory) > -1);  
      this.exampleItems = this.textCategory  === 'all' ? this.novelAll(this.All) : searchResult[0]['novel_datas']
      this.dataCategory =  this.textCategory  === 'all' ? this.novelAll(this.All) : searchResult[0]['novel_datas']
      this.sort()
    },
    novelAll(ojb:any){
      let NovelAll = [] as any
    ...
    sort(){
      ...
      if (TypeSort.top === this.dataSort){
        ...
        this.exampleItems.sort((a:any, b:any) =>  b.title.localeCompare(a.title))
      }
    },
    TypeNovel(type:string){
       if(this.translated === true){
          console.log();
       }
      // console.log('>>>>dataCategory ',);
      // this.dataCategory = this.exampleItems
      // console.log(this.dataCategory);
      // if(type === df){
      
      // }
      var searchResul = this.exampleItems.filter((uuid:any) => uuid['novel_data_type_id'].toLowerCase().indexOf(type) > -1);
      this.exampleItems= searchResul
    },
    async getRecommend(){
      ...
      res.data.data.forEach((element:any, index:number) => {
        if(element.novel_data){
          ...
          if(this.textCategory !== 'all'){
           ...
           if(element.novel_data.novel_category_data_id === this.textCategory){
              console.log(element.novel_data.novel_category_data_id === this.textCategory);
