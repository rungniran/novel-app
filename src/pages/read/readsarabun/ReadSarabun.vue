...
export default Vue.extend({
  data(){
    return{
        data: this.$store.state.sarabun.sarabun ? this.$store.state.sarabun.sarabun : null
    }
  },
  props:{
    uuid:String
  ...
  methods:{
    async getSarabun(){
      const resGetNovel = await Gatway.getIDService((this as any).cleck === "true" ? '/novel/novel-data' : '/guest/novel/novel-data',this.uuid);
